...
class UEditorForm(forms.Form):
    ...
    title = forms.CharField(label=u'标题',widget=forms.TextInput(attrs={'class': 'form-control', 'name':'title','placeholder':u'请输入项目标题'}))
    ...
    summary = forms.CharField(label=u'摘要',widget=forms.Textarea(attrs={'class': 'form-control','placeholder':u'请输入项目摘要'}))
    tags = forms.CharField(label=u'标签',widget=forms.TextInput(attrs={'class': 'form-control','placeholder':u'请输入项目标签，用逗号分隔，最多输入三个标签'}))
